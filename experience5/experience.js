document.addEventListener('DOMContentLoaded', () => {
  ...
  const stageMapping = [
      ...
      { name: "Bud break", visualTextIndex: 5, row: eduRows[1], imgRange: null, isHeader: false },
      { name: "Plant rootstock", visualTextIndex: 6, row: eduRows[2], imgRange: null, isHeader: false }
  ];
  ...
  function activateStage(idx, autoScrollToStage = false) {
      ...
      if (experienceRowWrapEl) {
          ...
          if (currentStageDetails.name === "Bud break") {
...
function setPatternedBackground(rowEl, imgUrl, options = {}) {
    const count = options.count || Math.floor(5 + Math.random()); // 5~6
    const width = options.width || Math.round(rowEl.offsetWidth || 540);
    const height = options.height || 200;
    const alpha = options.alpha || 0.20;
    const minDist = options.minDist || 80; // 이미지 중심끼리 최소 거리
    ...
    img.onload = () => {
        let attempts = 0, placedCount = 0, maxAttempts = count * 10;

        while (placedCount < count && attempts < maxAttempts) {
            const size = 48 + Math.random() * 44; // 48~92px
            // 테두리 안쪽에만 위치
            const x = Math.random() * (width - size - 20) + 10 + size/2;
            const y = Math.random() * (height - size - 20) + 10 + size/2;
            // 충돌(거리) 체크
            let tooClose = false;
            for (let i = 0; i < placed.length; i++) {
                ...
                if (dist < minDist + size/2 + placed[i].size/2) {
                    tooClose = true; break;
                }
            }
            if (!tooClose) {
        ...
        // 배경 패턴을 dataURL로 row에 적용
        rowEl.style.backgroundImage = 
          `url(${canvas.toDataURL("image/png")}), var(--glass-bg-light)`;
        rowEl.style.backgroundSize = `${width}px ${height}px, cover`;
        rowEl.style.backgroundRepeat = "no-repeat, no-repeat";
        rowEl.style.backgroundPosition = `center top, center center`;
    };
    img.src = imgUrl;
}
...
window.addEventListener('DOMContentLoaded', () => {
    if (typeof eduRows !== "undefined" && eduRows.length > 1) {
        setPatternedBackground(
            eduRows[1],
            'images/bud.png',
            { count: 5, height: 200, alpha: 0.22, minDist: 96 }
        );
    }
    if (typeof eduRows !== "undefined" && eduRows.length > 2) {
        setPatternedBackground(
            eduRows[2],
            'images/seed.png',
            { count: 6, height: 200, alpha: 0.22, minDist: 96 }
        );
    }
});