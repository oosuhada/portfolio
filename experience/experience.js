document.addEventListener('DOMContentLoaded', () => {
  ...
  const stageMapping = [
    ...
    { name: "Education Intro", visualTextIndex: -1, row: null, imgRange: [17, 17], currentImgLoopIdx: 0, loopIntervalId: null, isHeader: true },
    ...
    if (workExpHeaderFullEl && workExpHeaderStageIdx !== -1) {
        ...
        playVineLoadAnimationThenActivateFirstStage(() => {
            if (workExpHeaderStageIdx !== -1) {
                activateStage(workExpHeaderStageIdx, false); 
            }
            // 콜백의 가장 마지막에서 플래그 해제
  ...
  function updateVineGaugeLayout() {
    ...
    vineSideGaugeBlocks.forEach((block, index) => {
      ...
      if (blockTop + blockHeight > gaugeTrackDisplayHeight && blockTop > 0) { blockTop = Math.max(0, gaugeTrackDisplayHeight - blockHeight); }
  ...
  function setActiveHighlights(idx) {
    ...
    if (currentStage.name !== "WorkExpHeader" && currentStage.name !== "Intro" &&
  ...
  function startImageLoopForStage(stageIndex) {
    ...
    else if (stage.name === "WorkExpHeader") { imagesInLoop.push(25); } 
  ...
  function activateStage(idx, autoScrollToStage = false) {
    ...
    // autoScrollToStage가 true이고, (애니메이션 중 사용자가 스크롤하지 않았거나 또는 현재 자동 Hero 전환 중이 아닐 때)
    const shouldReallyAutoScroll = autoScrollToStage && 
                                 (!userInteractedDuringAnimation || !isAutoScrollingToWorkExp); 
                                 // sectionAndDotObserver에서 autoScrollLock으로 제어하므로 여기서는 userInteracted만 고려해도 될 수 있음.
                                 // 하지만 isAutoScrollingToWorkExp도 확인하여 Hero 전환 중 타임라인 클릭으로 인한 이중 스크롤 방지.
    ...
    if (shouldReallyAutoScroll) {
      ...
      if (targetElementToScroll && currentStage.name !== "Intro") {
        ...
        // 헤더거나 특정 중요 섹션, 또는 WorkExpHeader로의 초기 자동 스크롤 시에는 중앙 정렬 시도
        const shouldAlwaysScrollToCenter = currentStage.isHeader || 
                                         currentStage.name === "Plant rootstock" || 
                                         (currentStage.name === "WorkExpHeader" && isAutoScrollingToWorkExp); // Hero에서 내려올 때만 강제 중앙 정렬
    ...
    setTimeout(() => { 
        ...
        // isAutoScrollingToWorkExp는 triggerHeroToWorkExpSequence의 콜백에서 최종적으로 해제됨.
        // 여기서 추가로 해제할 필요는 없음.
    }, 800); 
  }
  ...
  if ('IntersectionObserver' in window && timelineRows.length > 0 && eduRows.length > 0 ) {
    const sectionAndDotObserverCallback = (entries) => {
      ...
      let bestEntry = null; let minDist = Number.POSITIVE_INFINITY;
      entries.forEach(entry => {
        const rect = entry.target.getBoundingClientRect(); const centerY = rect.top + rect.height / 2; const viewportCenter = window.innerHeight / 2; const dist = Math.abs(centerY - viewportCenter);
        const isCenterZone = centerY > window.innerHeight * 0.30 && centerY < window.innerHeight * 0.70;
        if (entry.isIntersecting && isCenterZone && dist < minDist) { minDist = dist; bestEntry = entry.target; }
      ...
      if (bestEntry) {
        ...
        if (idx !== -1 && stageMapping[idx].name !== "Intro") {
          activateStage(idx, false); // 타임라인 스크롤 시에는 활성화만 (기존 코드 방식)
  ...
  stageMapping.forEach((stage, index) => {
    const elementsToClick = []; if (stage.row) elementsToClick.push(stage.row);
    if (stage.name !== "WorkExpHeader" && stage.name !== "Intro" && stage.visualTextIndex >= 0 && stage.visualTextIndex < grapeTexts.length && grapeTexts[stage.visualTextIndex]) {
      elementsToClick.push(grapeTexts[stage.visualTextIndex]);
    }
    elementsToClick.forEach(element => {
      if (element) { element.style.cursor = 'pointer'; element.addEventListener('click', (event) => {
          event.stopPropagation(); 
          if (stage.name !== "Intro" && !isAutoScrollingToWorkExp) { // 자동 스크롤 중이 아닐 때만 클릭으로 인한 스크롤 허용
            userInteractedDuringAnimation = false; // 클릭은 명시적 행동이므로 플래그 초기화
            activateStage(index, true); // 클릭 시에는 해당 요소로 스크롤
          }
      }); }
    });
  });
  ...
  function eduScrollAction(event) { 
    ...
    userInteractedDuringAnimation = false;
    if (eduIntroStageMapIndex !== -1 && !isAutoScrollingToWorkExp) activateStage(eduIntroStageMapIndex, true); 
  }
  ...
  function createVineSideGauge() {
    ...
    if (vineSideGaugeBlocks.length > 0) { vineSideGaugeThumbEl = document.createElement('div'); vineSideGaugeThumbEl.className = 'vine-gauge-thumb'; vineSideGaugeTrackContainerEl.appendChild(vineSideGaugeThumbEl); }
    else { vineSideGaugeThumbEl = null; }
    setTimeout(updateVineGaugeLayout, 100); updateVineSideGaugeActiveStateStyles();
  }
  function updateVineSideGaugeActiveState(currentActiveStageMapIndex) { const newVineBlockIndex = vineGaugeStageMapIndices.indexOf(currentActiveStageMapIndex); currentVineGaugeActiveBlockIndex = newVineBlockIndex !== -1 ? newVineBlockIndex : -1; updateVineSideGaugeActiveStateStyles(); updateVineSideGaugeThumbPosition(); }
  function updateVineSideGaugeActiveStateStyles() { /* No direct styling for blocks */ }
  function updateVineSideGaugeThumbPosition() {
    ...
    if (activeBlock) {
      requestAnimationFrame(() => {
        const blockHeight = activeBlock.offsetHeight; const blockTop = activeBlock.offsetTop;
        if (blockHeight > 0) { vineSideGaugeThumbEl.style.top = `${blockTop}px`; vineSideGaugeThumbEl.style.height = `${blockHeight}px`; vineSideGaugeThumbEl.style.opacity = '1'; }
        else { vineSideGaugeThumbEl.style.opacity = '0'; }
      });
    } else { if(vineSideGaugeThumbEl) vineSideGaugeThumbEl.style.opacity = '0'; }
  }
