// script/career.js
// 페이지 전환 및 전역 상태 관리를 위한 주 진입점 및 오케스트레이터 (수정된 버전)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ✨ [테마 개선] 테마 초기화 및 동적 요소(AI 버튼, 커서) 강제 동기화
    // common.js에서 이미 호출되므로 여기서는 중복 호출을 방지합니다.
    if (window.themeManager) window.themeManager.initialize();
    // if (typeof initializeAIAssistantButton === 'function') initializeAIAssistantButton(); // 이 라인이 중복 호출을 일으키므로 제거
    if (typeof updateAIAssistantAskImage === 'function') updateAIAssistantAskImage();
    if (typeof updateCursorVarsByTheme === 'function') updateCursorVarsByTheme();
