document.addEventListener('DOMContentLoaded', () => {
  ...
  // const grapeImg = document.getElementById('vine-stage-img'); // REMOVED
  const grapeTexts = document.querySelectorAll('.fade-in-list > div');
  const timelineRows = document.querySelectorAll('.timeline-row');
  const eduRows = document.querySelectorAll('.edu-row');
  const experienceRowWrapEl = document.querySelector('.experience-row-wrap'); // NEW background target
  ...
  // let vineAnimationCurrentlyPlaying = false; // Related to grapeImg, review if needed for other animations
  ...
  // ===================== Stage Mapping (imgRange still used for background) =====================
  const stageMapping = [
      ...
      { name: "Ripening", visualTextIndex: 1, row: timelineRows[0], imgRange: [23, 24], isHeader: false },
      ...
      { name: "Bud break", visualTextIndex: 5, row: eduRows[1], imgRange: [5, 11], isHeader: false },
      { name: "Plant rootstock", visualTextIndex: 6, row: eduRows[2], imgRange: [1, 4], isHeader: false }
  ];
  ...
  if (experienceRowWrapEl) {
      experienceRowWrapEl.style.backgroundImage = `url('images/25.png')`; // Default starting background
  }
  ...
  function triggerHeroToWorkExpSequence() {
      ...
      setTimeout(() => { // Release lock after a delay
          isAutoScrollingToWorkExp = false;
      }, 1000); // Adjust delay as needed for scroll to complete
  }
      ...
      const contentWrapperObserver = new IntersectionObserver(entries => {
          entries.forEach(entry => {
              ...
              if (entry.isIntersecting &&
                  ...
                  // !vineAnimationCurrentlyPlaying && // Removed as grapeImg animation is gone
  ...
  lastScrollY = window.scrollY; // Already declared, ensure it's in the correct scope
      ...
      if (navHeader) {
          if (zoomState === 'zoomOut') {
              ...
              if (currentScrollY < lastScrollY && currentScrollY < 50) {
                  ...
                  // navHeader.classList.add('experience-header-hidden-override'); // Decided against auto-hiding on scroll down while zoomed
              }
          }
      }
      lastScrollY = currentScrollY <= 0 ? 0 : currentScrollY;
      const currentFooterImgEl = document.getElementById('grapeFooterImg'); // Renamed to avoid conflict
          if (plantRootstockStageActive &&
          !hasScrolledToFooterAfterPlantRootstock &&
          zoomState !== 'zoomIn' &&
          currentFooterImgEl && // Use renamed variable
          ...
          if (lastEduRowRect.bottom < viewportHeight * 0.25 &&
              ...
              const footerRect = currentFooterImgEl.getBoundingClientRect(); // Use renamed variable
  ...
  function activateStage(idx, autoScrollToStage = false) {
      ...
      if (experienceRowWrapEl && currentStageDetails.imgRange) {
          let bgImageNumber;
  ...
  function createVineSideGauge() {
      ...
      vineSideGaugeEl = document.getElementById('vineSideGauge'); // Try to find existing
      if (!vineSideGaugeEl) { // If not found, create it
  ...
  const footerImgElGlobal = document.getElementById('grapeFooterImg'); // Already declared
  const footerTextElGlobal = document.getElementById('grapeFooterText'); // Already declared
  ...
  window.addEventListener('load', () => {
      ...
      if(stageMapping.length > 0 && stageMapping[0].name === "Intro" && typeof activateStage === 'function') {
         // activateStage(0, false); // Sets initial background to image 25. You can uncomment if desired.
      }
  });
  ...
  // Hero setup is called at the end of the script, after all functions are defined.
  if (heroBg) {
      hideScrollDownButton();
      initialHeroSetup();
  }
