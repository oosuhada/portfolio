...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (timelineSectionContainer && window.timelineComponent) {
        timelineSectionScrollTrigger = ScrollTrigger.create({
            ...
            onEnter: () => {
                ...
                // 텍스트 히어로 컴포넌트의 활성화/비활성화 로직은 이제 textHeroAreaObserver에 의해 독립적으로 관리됩니다.
                orchestrateHeaderVisibility(); // 헤더 가시성 오케스트레이션
                console.log("디버그: 타임라인 ScrollTrigger onEnter 발생.");
            },
            onLeaveBack: () => {
                // 스크롤을 위로 올려 타임라인이 다시 뷰포트를 벗어날 때
            ...
            onEnterBack: () => {
                // 스크롤을 다시 아래로 내려 타임라인이 뷰포트에 다시 진입할 때
                window.timelineComponent.renderSection(globalState.currentActiveTimelineSection); // 타임라인 섹션 렌더링
                setGlobalState({ isTimelineAreaVisible: true }); // 타임라인 영역 가시성 전역 상태 업데이트
                // 텍스트 히어로 컴포넌트의 활성화/비활성화 로직은 이제 textHeroAreaObserver에 의해 독립적으로 관리됩니다.
                orchestrateHeaderVisibility(); // 헤더 가시성 오케스트레이션
            ...
            onLeave: () => {
                // 스크롤을 아래로 내려 타임라인이 뷰포트를 완전히 벗어날 때
                setGlobalState({ isTimelineAreaVisible: false }); // 타임라인 영역 가시성 전역 상태 업데이트
                orchestrateHeaderVisibility(); // 헤더 가시성 오케스트레이션
    ...
    // REMOVED: 포도 푸터에 대한 IntersectionObserver는 더 이상 필요하지 않습니다.
    // 모든 애니메이션은 이제 grapefooter.js의 ScrollTrigger에 의해 처리됩니다.
    ...
    window.addEventListener('scroll', () => {
        ...
        // 부드러운 스크롤 중 사용자가 직접 스크롤했는지 감지 (5px 이상 움직임)
    ...
    /**
     * 경험 헤더 테마를 토글합니다 (다크/라이트 모드).
     */
    function toggleExperienceHeaderTheme() {
        const currentTheme = document.body.getAttribute('data-theme'); // 현재 테마 가져오기
        const newTheme = currentTheme === 'dark' ? 'light' : 'dark';   // 새 테마 결정
        document.body.setAttribute('data-theme', newTheme);             // body에 data-theme 속성 설정
    ...
    if (!savedTheme || savedTheme === 'light') { // 저장된 테마가 없거나 'light'면
        savedTheme = 'dark'; // 기본값을 'dark'로 설정
        localStorage.setItem('theme', savedTheme);
    }
    document.body.setAttribute('data-theme', savedTheme); // body에 저장된 테마 적용

    // 테마에 따라 초기 아이콘 가시성 설정
    if (savedTheme === 'light') {
